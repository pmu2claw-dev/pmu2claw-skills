...
OUT = '/home/user/workspace-tenji/JD6628H_SinglePwr_2-1_to_2-9_SplitRows_TenjiV2.2_R0.2.xlsm'
...
def step_symbol(no, step_idx, primary_v):
    if step_idx == 1:
        return f'SP_{no.replace("-", "_")}_C1_ONLY_{primary_v}V'
    if step_idx == 2:
        return f'SP_{no.replace("-", "_")}_C1_C2_SHARE_5V'
    if step_idx == 3:
        return f'SP_{no.replace("-", "_")}_C1_RECOVER_{primary_v}V'
    raise ValueError(step_idx)
...
if 'Revision history' in wb.sheetnames:
    ...
    rev_ws.cell(rev_row, 4).value = 'Repair JD6628H 單電源2-1~2-9 split-row TENJI workbook: restore workbook head/tail OS_test and fill SPEC columns'
