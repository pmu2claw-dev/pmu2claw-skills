#!/usr/bin/env python3
"""
TENJI durable memory-driven validator rules.
"""
...
def apply_memory_rules(spec: Dict[str, Any]) -> Dict[str, Any]:
    errors: List[str] = []
    warnings: List[str] = []

    memory_context = build_memory_brief(_spec_query(spec), limit=6, max_chars=1600)
    ...
    for item in spec.get("test_items", []):
        ...
        if _is_protocol_like(item):
            # 將協議測項的 Attach/Detach 檢查從 Error 降級為 Warning，因為連續流程中中間步驟不需 Attach
        ...
        if _is_os_item(item):
            # 只有當 Symbol 確實是 OS 開頭時才強制執行固定格式檢查，避免誤殺含有 'short' 關鍵字的普通描述
            symbol = str(item.get("symbol") or "").upper()
            if symbol.startswith("OS"):
                extra_commands = bool(item.get("commands"))
                extra_fields = any(str(item.get(field) or "").strip() for field in ["measure_cond", "pseudo_code", "run_pattern"])
